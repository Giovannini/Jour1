...
class TileTest extends FunSuite {
    ...
    val c2 = Concept("C2", List())
    val i1 = Instance("I1", Coordinates(0, 0), List(), c1)
    val i2 = Instance("I2", Coordinates(0, 0), List(), c2)
    ...
    val fullTile = Tile(Coordinates(17, 5), List(i1))
    ...
    test("method hasConcept"){
        assert(fullTile.hasConcept(c1))
        assert(! fullTile.hasConcept(c2))
    }
