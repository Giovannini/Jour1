 ...
 * Time: 23:25
 */
object SocketTestController extends Controller with Secured{
  ...
  /**
   * This function create a WebSocket using the enumerator linked to the current user and retrieved from TaskActor.
   */
  def indexWS = withAuthWS {
    userId =>
      ...
      (timerActor ? StartSocket(userId)).map {
        /* Create an Iteratee ignoring the input and sending a SocketClosed message to the actor when
           connection is closed from the client. */
        enumerator =>
          (Iteratee.ignore[JsValue]
            .map(_ => timerActor ! SocketClosed(userId)),
    ...
    implicit request =>
      Ok(
        Routes.javascriptRouter("jsRoutes")(
          routes.javascript.SocketTestController.indexWS,
          routes.javascript.SocketTestController.start,
          routes.javascript.SocketTestController.stop
        )
      ).as("text/javascript")
  }
...
trait Secured {
  ...
  def unauthF(request: RequestHeader) = {
    ...
    Redirect(routes.SocketTestController.index()).withSession(Security.username -> newId)
  }
